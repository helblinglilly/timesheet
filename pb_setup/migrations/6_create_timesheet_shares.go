...
func init() {
	m.Register(func(app core.App) error {
		existing, _ := app.FindCollectionByNameOrId("timesheet_shares")
		if existing != nil {
			return nil
		}
		...
		collection := core.NewBaseCollection("timesheet_shares")
		...
		collection.Indexes = []string{
			"CREATE UNIQUE INDEX `idx_hQ1DrHLQP7` ON `timesheet_shares` (`user_email`, `timesheet`)",
		}

		return app.Save(collection)
	}, func(app core.App) error {
		collection, err := app.FindCollectionByNameOrId("timesheet_shares")
		if err != nil {
			return err
		}
